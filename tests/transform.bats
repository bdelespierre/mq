...
@test "process_argument: %json without argument returns error" {
    local out="" shift_count=""
    run bash -c 'source "$1" && process_argument out shift_count %json' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
...
@test "process_argument: %string without argument returns error" {
    local out="" shift_count=""
    run bash -c 'source "$1" && process_argument out shift_count %string' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
...
@test "process_argument: %j without argument returns error" {
    local out="" shift_count=""
    run bash -c 'source "$1" && process_argument out shift_count %j' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
...
@test "process_argument: %s without argument returns error" {
    local out="" shift_count=""
    run bash -c 'source "$1" && process_argument out shift_count %s' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
...
@test "process_argument: %in without values returns error" {
    local out="" shift_count=""
    run bash -c 'source "$1" && process_argument out shift_count %in from' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
...
@test "build_query: trailing %json without argument returns error" {
    run bash -c 'source "$1" && build_query sql vertical select %json' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
    [ "$output" = "error: %json requires an argument" ]
}

@test "build_query: trailing %string without argument returns error" {
    run bash -c 'source "$1" && build_query sql vertical select %string' -- "$PROJECT_ROOT/lib/mysql-query/transform.sh"
    [ "$status" -eq 1 ]
    [ "$output" = "error: %string requires an argument" ]
}
